...
ACs execute the handover procedure every $t$ seconds, pausing transmissions for $p$ seconds as a penalty for this operation.%accordingly with the following algorithm:
%\begin{verbatim}
...
%\texttt{TODO scrivere qui perché abbiamo scelto M=25km, T=4.3e-9 eccetera, vedere Parametri.xlsx}
...
\end{itemize}
Moreover, we would like to check for an handover neither when the AC is too close to a cell edge, nor when it is too far.
Fixing these values at a reasonable distance of 1km and 4km, and dividing the interval in 5 chunks, we find these possible values for $t$: $\left\{4.24s, 7.42s, 10.59s, 13.77s, 16.95s\right\}$.
...
\begin{figure}[H]
  \includegraphics{img/warmup-study.pdf}
...
\subsection{Single-BS distance distribution computation}
Using results from Section~\ref{sec:singlebs-ddc}, we ran 30 independent simulations and sampled distance and service time.
Then we compared the samples distributions with our theoretical distributions using QQ plots.
...
As we can see from Figure~\ref{fig:qqplots}, this experiment confirmed the validity of our simulator.

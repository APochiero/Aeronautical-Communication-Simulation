...
\subsection{Maximum distance considerations}
Using results from Section~\ref{sec:max-distance-considerations}, we ran 10 indpendent simulations, with 5 different handover periods $t$, and sampled maximum distance. Then we compared the samples with out theoretical computations, and, as we can see from Figure~\ref{fig:dmax-validation}, the experiment always confirmed the validity of our simulator.
...
\subsection{Single-BS distance distribution computation}
Using results from Section~\ref{sec:singlebs-ddc}, we ran 30 independent simulations and sampled distance and service time. Then we compared the samples distributions with our theoretical distributions using QQ plots.
